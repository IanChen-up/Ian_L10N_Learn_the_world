        ...
        >
          ...
          <div className="p-6">
            {points === null ? (
              ...
              <Chart points={points} locale={locale} />
            )}
...
function Chart({ points, locale }: { points: Point[]; locale: string }) {
  const { t } = useTranslation();
  const W = 600;
  const H = 260;
  const padL = 56;
  ...
  return (
    ...
    >
      ...
      {[points[0], points[points.length - 1]].map((p, i) => (
        <g key={i}>
          <circle cx={x(p.year)} cy={y(p.value)} r={2.6} fill="hsl(var(--accent))" />
        </g>
      ))}
        ...
        <g pointerEvents="none">
          ...
          <g transform={`translate(${tipLeft}, ${padT + 4})`}>
            ...
            <text x={8} y={27} fontSize={11} fontWeight={600} fill="hsl(var(--text-primary))">
              {formatGDP(hover.value, locale)}
            </text>
          </g>
        </g>
      )}

      {/* 透明命中层：保证整幅图都能捕获指针（放最后避免遮挡视觉元素的事件） */}
